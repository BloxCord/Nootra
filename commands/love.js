...
module.exports = {
    ...
    execute(client, message, args) {
        ...
        if (size > 1) var result = percent <= 30 ? `${usermention.user.username}, you should'nt mess with ${usermention2.user.username}..` : percent <= 50 ? `You could at least try to be friends ?` : percent <= 70 ? 'Close friends' : percent <= 80 ? "Best friends" : percent <= 90 ? 'Inseparable' : percent <= 100 ? 'Perfect Match!' : "You're made for yourself!";
        else var result = percent <= 30 ? `${user.user.username}, you should'nt mess with ${usermention.user.username}..` : percent <= 50 ? `You could at least try to be friends ?` : percent <= 70 ? 'Close friends' : percent <= 80 ? "Best friends" : percent <= 90 ? 'Inseparable' : percent <= 100 ? 'Perfect Match!' : "You're made for yourself!";
