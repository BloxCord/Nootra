...
exports.run = (client, message, args) => {
    ...
    if (message.author.id === config.admin || user.hasPermission('MENTION_EVERYONE')) {
        ...
        const embed = new Discord.RichEmbed()
            ...
            .setTimestamp();
        if (args[1]) {
            embed.addField('Proposition n°1 :', args[1], true);
        }
        if (args[2]) {
            embed.addField('Proposition n°2 :', args[2], true);
        }
        if (args[3]) {
            embed.addField('Proposition n°3', args[3], true);
        }
        if (args[4]) {
            embed.addField('Proposition n°4', args[4], true);
        }
        if (args[5]) {
            embed.addField('Proposition n°5', args[5], true);
        }
        if (args[6]) {
            embed.addField('Proposition n°6', args[6], true);
        }
        ...
        return message.reply("you don't have access to this command.exports.timer = (client) => ms('2seconds');exports.timer = (client) => ms('2seconds');");
    }
};